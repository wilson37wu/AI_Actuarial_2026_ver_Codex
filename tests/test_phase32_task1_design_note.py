"""Tests for Phase 32 Task 1 - UI consolidation design note + gate."""
...
class TestDesignNoteStructure(unittest.TestCase):
    ...
    def test_three_gaps_in_priority_order(self):
        gaps = self.note["gaps"]
        self.assertEqual([g["gap_id"] for g in gaps], ["G1", "G2", "G3"])
    ...
    def test_every_gap_is_additive_with_criteria(self):
        for g in self.note["gaps"]:
            ...
            self.assertGreaterEqual(len(g["acceptance_criteria"]), 7)

    def test_g1_pins_pack_fidelity_and_neutrality(self):
        crits = " | ".join(self.note["gaps"][0]["acceptance_criteria"])
        self.assertIn("PHASE31_TASK2_OWNER_DECISION_PACK", crits)
...
class TestGate(unittest.TestCase):
    ...
    def test_gate_passes_against_repo(self):
        ...
        self.assertGreaterEqual(gate["n_checks"], 18)
