#!/usr/bin/env python3
"""Build the Phase 19 Task 4 FIVE-driver tail-convergence/stability evidence.

Runs outer-count convergence, bootstrap CI/SE on 99.5% VaR/ES, and a
crude/antithetic/Sobol variance-reduction comparison on the five-driver
(rate + equity + credit-spread + lapse-behaviour + mortality-trend) LSMC
surface; writes docs/validation/PHASE19_TASK4_TAIL_DIAGNOSTICS_REPORT.{json,md}.

Run: PYTHONPATH=/var/tmp/pylibs:. python3 scripts/build_phase19_task4_tail_diagnostics.py
"""
...
def main() -> int:
    ...
    cfg = FiveDriverTailConfig(
        ...
        vr_replications=60, vr_n_outer=2_048, vr_pilot_n=8_000,
    )
    report = FiveDriverTailDiagnostics(product).run(config=cfg)
