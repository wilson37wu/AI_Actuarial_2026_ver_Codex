"""
Four-Driver Tail-Dependent Risk Aggregation for the Economic-Capital Proxy
==========================================================================

Phase 18 Task 4 (aggregation half).  Generalises the Phase 17 Task 3
*three-driver* variance-covariance aggregation and the Phase 18 Task 1
*copula* (tail-dependent) aggregation to the **four-driver** economic-capital
proxy ``(rate, equity-guarantee, credit-spread, lapse-behaviour)`` of
:mod:`par_model_v2.projection.multi_driver_capital_4d`.

What it produces
----------------
For one shared set of governed, 4-factor-correlated outer states ``(r,S,s,b)``
at the capital horizon H it isolates **four** standalone capital-loss vectors by
a common-random-number (CRN) decomposition of the conditional liability, then:

  1. aggregates the four standalone SCRs with the governed **4x4 ESG factor
     correlation** (variance-covariance / second-moment formula) - the legacy
     method whose understatement is **MR-010**;
  2. re-aggregates the four realised standalone capital-loss vectors with a
  ...
  3. benchmarks both against the **genuine four-driver nested capital**, computed
     on the SAME outer states and inner seeds (all four drivers ON), so the
     comparison is exact like-for-like.

The fourth driver - lapse behaviour - enters the policyholder benefit
**multiplicatively** through the in-force factor ``IF(r,b)`` (it is *not* an
additive component like rate/equity/credit).  The CRN decomposition therefore
carries a small but genuine **interaction residual** (the multiplicative
cross-term ``IF*equity`` that a first-order additive split cannot reproduce),
which this module measures and discloses rather than hides.  A positive residual
means the genuine nested capital is SUPER-additive vs the CRN-additive standalone
sum, so 'nested <= standalone sum' is NOT a valid invariant for four drivers.
This is the new four-driver finding for MR-010.

The module is **additive**: it imports, but never modifies, the four-driver
nested primitives (:mod:`...multi_driver_capital_4d`) and the Phase 18 Task 1
copula aggregator (:mod:`...multi_driver_copula_aggregation`).

SOA ASOP 56 §3.5; SOA ASOP 25 §3.3; SOA ASOP 7 §3.3; IA TAS M §3.2/§3.6;
Solvency II Delegated Reg. Art. 234; IFoA Life Aggregation & Simulation WP.
"""
...
DEFAULT_FOURD_AGG_GAP_TOLERANCE = 0.35
...
class _NoLapseExposure(LapseExposureSpec):
    """A lapse-exposure whose in-force factor is identically 1.0.

    Used to switch the lapse driver OFF in the CRN decomposition: the benefit
    components are evaluated on the *central* (no behavioural-lapse) basis so the
    isolated lapse component is ``L_lapse = IF(r,b)*guaranteed - guaranteed``.
    """
...
@dataclass
class FourDriverAggregationConfig:
    """Configuration for the Phase 18 Task 4 four-driver aggregation run."""
    ...
    aggregation_gap_tolerance: float = DEFAULT_FOURD_AGG_GAP_TOLERANCE
...
@dataclass
class FourDriverStandaloneCapital:
    """The four CRN-isolated standalone capital figures + realised loss corr."""
...
@dataclass
class FourDriverVarCovarAggregation:
    """Variance-covariance (4x4 ESG factor) aggregation vs nested benchmark."""
...
@dataclass
class FourDriverAggregationReport:
    """Full structured Phase 18 Task 4 four-driver aggregation report."""

    config: FourDriverAggregationConfig
    drivers: Tuple[str, ...]
    standalone: FourDriverStandaloneCapital
    var_covar: FourDriverVarCovarAggregation
    ...
    def to_markdown(self) -> str:
        ...
        lines = [
            ...
            "|--------|--------------:|---------------------:|",
            "| Var-covar (4x4 ESG factor) | {:,.1f} | {:.1%} |".format(
            ...
            "in-force x equity-guarantee cross-term the additive split omits.".format(
                vc.interaction_residual_scr, vc.interaction_residual_rel),
            "",
            "## Notes",
            "",
        ]
...
class FourDriverRiskAggregator:
    """Four-driver standalone capital, var-covar + copula aggregation, nested benchmark.

    Reuses the Phase 18 Task 3 four-driver nested primitives and the Phase 18
    ...
    def _component_liabilities(
        self, outer: np.ndarray, cfg: FourDriverAggregationConfig
    ) -> Tuple[np.ndarray, np.ndarray, np.ndarray, np.ndarray, np.ndarray]:
        """CRN-isolated rate / equity / credit / lapse + genuine full liability.

        For each outer state ``(r,S,s,b)`` the SAME inner seed drives five
        valuations, so the inner ``(rate, equity, spread)`` paths are identical:

            L_base   = guaranteed_pv                      (eq OFF, cr OFF, lapse OFF/IF=1)
            L_re     = guaranteed_pv + equity_pv          (eq ON,  cr OFF, lapse OFF)
            L_rc     = guaranteed_pv + credit_pv          (eq OFF, cr ON,  lapse OFF)
            L_rl     = IF(r,b) * guaranteed_pv            (eq OFF, cr OFF, lapse ON)
            L_full   = IF(r,b)*(guaranteed_pv+equity_pv)
                       + credit_pv                        (all ON; genuine nested)
        ...
        The four additive components do NOT sum to ``L_full`` because lapse acts
        multiplicatively (the ``IF*equity`` cross-term); the residual is reported.
        """
        ...
        for i, (r, s, c, b) in enumerate(outer):
            ...
            def _mean(eq_spec, cr_spec, lapse_spec):
                pvs = _inner_pathwise_pvs_4d(
                    float(r), float(s), float(c), float(b), cfg.n_inner, rem,
                    self.product, self.hw_params, self.gbm_params, self.spread_params,
                    self.correlation, cfg.capital_horizon_months, inner_seed,
                    eq_spec, cr_spec, lapse_spec, self.annual_qx_fn,
                )
                return float(pvs.mean())

            l_base = _mean(eq_off, cr_off, lapse_off)
            l_re = _mean(eq_on, cr_off, lapse_off)
            l_rc = _mean(eq_off, cr_on, lapse_off)
            l_rl = _mean(eq_off, cr_off, lapse_on)
            l_full = _mean(eq_on, cr_on, lapse_on)
            rate_l[i] = l_base
            equity_l[i] = l_re - l_base
            credit_l[i] = l_rc - l_base
            lapse_l[i] = l_rl - l_base
            full_l[i] = l_full
        return rate_l, equity_l, credit_l, lapse_l, full_l

    def run(
        self,
        config: Optional[FourDriverAggregationConfig] = None,
        governance_store: Optional["object"] = None,
        actor: str = "FourDriverRiskAggregator",
        phase: str = "Phase 18: Tail-Dependent Risk Aggregation and Driver/Calibration Sophistication",
    ) -> FourDriverAggregationReport:
        cfg = config or FourDriverAggregationConfig()
        ...
        t0 = time.monotonic()
        run_id = "fd-riskagg-" + uuid.uuid4().hex[:8]
        ...
        rate_l, equity_l, credit_l, lapse_l, full_l = self._component_liabilities(outer, cfg)
        crn_sum_l = rate_l + equity_l + credit_l + lapse_l
        ...
        # Realised 4x4 capital-loss correlation.
        stacked = np.vstack([rate_l, equity_l, credit_l, lapse_l])
        ...
        scr_vec = np.array(
            [rate_cap.scr_proxy, equity_cap.scr_proxy,
             credit_cap.scr_proxy, lapse_cap.scr_proxy], dtype=float
        )
        standalone_sum = float(scr_vec.sum())

        # Governed 4x4 ESG driver correlation (rate, equity, credit, lapse).
        ...
        # loss vectors, benchmarked to the genuine four-driver nested capital.
        copula_cfg = CopulaAggregationConfig(
            n_sim=cfg.n_sim_copula, seed=cfg.seed + 7,
            confidence_level=conf, capital_horizon_months=hm,
        )
        copula = CopulaRiskAggregator(
            loss_vectors=[rate_l, equity_l, credit_l, lapse_l],
            driver_names=["rate", "equity", "credit", "lapse"],
        ...
        notes: List[str] = [
            ...
            "MR-010 (four-driver refresh): the raw ESG-factor var-covar formula understates the diversified nested capital by "
            "{:.1f}% because the realised capital-loss vectors co-move positively in the tail while several ESG factor "
            "off-diagonals are negative.".format(100.0 * understatement_pct),
            "Copula-on-realised-losses (selected: {}) reconciles to nested capital within {:.1f}% - the implemented MR-010 "
            "mitigation, now extended to four drivers.".format(
                copula.selected_copula, 100.0 * copula.selected.scr_rel_error_vs_nested),
            "Four-driver finding: the CRN additive decomposition leaves a {:.1f}%-of-nested interaction residual because the "
            "lapse driver scales the benefits MULTIPLICATIVELY (the IF x equity-guarantee cross-term), unlike the exactly "
            "additive three-driver split. A positive residual means the genuine nested capital is SUPER-additive vs the "
            "CRN-additive standalone sum, so 'nested <= standalone sum' is NOT a valid invariant for four drivers.".format(
        ...
        digest = hashlib.sha256(
            np.round(
                np.concatenate([
                    full_l, rate_l, equity_l, credit_l, lapse_l,
        ...
        if governance_store is not None:
            try:
                ...
                entry = AuditEntry.model_run(
                    ...
                    scenario_count=cfg.n_outer * cfg.n_inner * 5,
                    ...
                    test_summary=(
                        ...
                        "var-cov={:.1f} (rel {:.1%}); copula({})={:.1f} (rel {:.1%}); "
                        "nested={:.1f}; ESG understatement={:.1f}%".format(
                            rate_cap.scr_proxy, equity_cap.scr_proxy,
                            credit_cap.scr_proxy, lapse_cap.scr_proxy,
                            correlated_scr, rel_err, copula.selected_copula,
        ...
        return FourDriverAggregationReport(
            config=cfg,
    ...
    def _verdict(
        ...
        checks = [
            ...
            # (always true for a valid correlation matrix).  Note: the GENUINE
            # nested capital is NOT so bounded for four drivers - the
            # multiplicative lapse in-force coupling can make it super-additive
            # vs the CRN-additive standalone sum (see interaction_residual).
        ...
        if all(checks):
            return (
                "PASS - four-driver copula aggregation (selected: {}) reconciles to nested capital "
                "within {:.1%} vs var-covar {:.1%}; MR-010 four-driver mitigation confirmed".format(
                    copula.selected_copula, sel.scr_rel_error_vs_nested,
                    var_covar.formula_vs_nested_scr_rel_error)
            )
        return "PARTIAL - four-driver aggregation evidence generated with review items"


def four_driver_aggregation_use_restrictions() -> Dict[str, object]:
    return {
        "module": "par_model_v2/projection/multi_driver_capital_4d_aggregation.py",
        "classification": "EDUCATIONAL ONLY - NOT a regulatory capital model",
        "risk_drivers": ["short rate", "equity guarantee", "credit spread", "lapse behaviour"],
        "method": (
            "Four standalone SCRs isolated by common-random-number decomposition of the "
            "conditional liability, aggregated BOTH with the governed 4x4 ESG factor "
            ...
            "AIC-selected), each benchmarked to genuine four-driver nested capital."
        ),
        "limitations": [
            "The CRN additive split carries a multiplicative-lapse interaction residual (the IF x equity-guarantee cross-term); it is reported, not removed.",
            ...
            "Four drivers only (rates + equity + credit + lapse): mortality-trend, FX, liquidity, and management-action risks remain outside the aggregation.",
...
def four_driver_aggregation_use_restrictions_json() -> str:
    return json.dumps(four_driver_aggregation_use_restrictions(), indent=2, sort_keys=True)
