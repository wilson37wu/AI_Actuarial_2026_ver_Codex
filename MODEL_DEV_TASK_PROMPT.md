...
>
> **NEXT-EXECUTION POINTER (W67).** The auto-admissible forward step is **stage-4b wiring**: expose the W66 `stratified_normal_outer_sampler` (and the opt-in `es_bias_corrected`) as a selectable **variance-reduction MODE** on the opt-in tail path / `engine ... inner_estimator` diagnostics (default OFF → governed headline byte-identical), gated by a **frozen-snapshot equivalence** check (mirrors the W60 stage-3 wiring pattern). This is **auto-runnable and does NOT re-baseline the headline**. If the owner has instead replied with a pivot, execute it: **A** MR-LONGEV-1 [model-form, sign-off] / **B** LSMC [sign-off] / **C** Phase IGUI [auto; owner's stated exclusive next major] / **D** Packaging A/B/C [auto] / **E** FREEZE. If neither, do a SINGLE verification pass + re-send the brief — do NOT add a near-duplicate graphic and do NOT make any model-FORM change. **Stage 5** (any tail-MLMC figure as the governed default) needs **owner sign-off + a fresh frozen reference**. Stage-4 evidence: `docs/validation/MLMC_TAIL_STAGE4_VALIDATION_20260619.md`. Design note: `docs/research/MLMC_QUANTILE_ESTIMATOR_DESIGN_NOTE_20260619.md`. Decision matrix: `docs/research/MODEL_IMPROVEMENT_RESEARCH_20260617.md`. Authoritative in_progress pointer = `.claude-dev/MODEL_DEV_STATE.json`.
>
---
