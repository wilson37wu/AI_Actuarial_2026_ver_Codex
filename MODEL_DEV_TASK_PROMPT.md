...
The RED-test backlog is cleared. **PHASE 36 is in_progress.** *Offline UI Accessibility Completion & Educational Reproducibility*. Task 1 (design note, gate PASS 29), **Task 2 / gap E1 (live-region status announcements)** and **Task 3 / gap E2 (consolidated global glossary & methodology explainer) are COMPLETE**. Task 3 / E2 this cycle — one polite `sr-only` `#srlive` region + `announce()` wired to tab/search/slider/integrity; ARIA/JS only, NO contract change (1.20.0; payload byte-identical, A2 digests verify); +10 self-test checks (368→378); all 8 offline self-tests ok (**473→483** checks, 0 net/0 JS err); 0 external refs; governed headline bit-for-bit; ChangeRecord `b274a0e0c43d4cd5affd5affbce45ec9` OWNER_REVIEW (governance **97/125/17**). Evidence: `docs/validation/PHASE36_TASK2_E1_REPORT.{json,md}`; card `docs/LIVE_REGION_ANNOUNCEMENTS_CARD.md`. Design note baseline (frozen): `docs/validation/PHASE36_TASK1_DESIGN_NOTE.md`.

The next agent should do EXACTLY ONE task, the single `in_progress` item:

1. **Phase 36 Task 3 = E2 — COMPLETE (2026-06-14, cycle 27a4).** Global build-time `explainer` key (23 terms, 18-tab coverage, verbatim limitation/glossary roots) + read-only "Methodology & Glossary" tab; ADDITIVE contract 1.20.0→1.21.0; +15 self-test checks (483→498, 8/8 green); ChangeRecord `514e5c20…87ff` OWNER_REVIEW. Evidence `docs/validation/PHASE36_TASK3_E2_REPORT.{json,md}`. **NEXT (the single `in_progress` item) = Phase 36 Task 4 = E3 — consolidated glossary & methodology explainer surface (superseded; see Task 4 below).** Promote the sign-off-pack-scoped `glossary` to a GLOBAL, build-time-assembled glossary / data-dictionary covering every governed read-out across the 18 tabs: each entry = term + plain-language definition + method/assumption basis + limitation provenance (carried VERBATIM from `owner_decision_p31.limitations` and the archived design notes — nothing re-derived). Surface as a read-only "Methodology & Glossary" panel; display-only, no recompute. ADDITIVE contract bump (1.20.0 → next minor) adding an `explainer` (or `glossary_global`) key ONLY; every pre-existing key renders bit-identically. Add self-test checks (global glossary presence, per-tab coverage, provenance carry-through). Acceptance criteria PRE-REGISTERED in the design note (E2).
2. **Phase 36 Task 4 = E3 — COMPLETE (2026-06-14, cycle a88a).** "Reproducibility evidence pack" toolbar button (`btnEvidencePack` → `exportEvidencePack()`) downloads the EXACT embedded `ui_data` bytes (byte-identical to `ui_data.json`; carries `contract_manifest` section+root SHA-256 digests + build/provenance stamp) as `reproducibility_evidence_pack_v<contract>_<root8>.json`; no network/no storage, `file://` safe; DISPLAY/JS only, NO contract change (1.21.0); governed headline bit-for-bit. +12 ui_app self-test checks (393→405); NEW jsdom fallback test proves byte+digest equality via the EXISTING in-browser verifier; all 9 self-tests ok (498→522); pytest 14 passed + 21 regression. ChangeRecord `d9cab0e6…ecc6` OWNER_REVIEW (records 98→99, audit 126→127). Evidence `docs/validation/PHASE36_TASK4_E3_REPORT.{json,md}`. **NEXT (the single `in_progress` item) = Phase 36 Task 5** — phase summary + consolidated baseline re-audit (self-test counts, external-ref scan, contract inventory) → PHASE 36 COMPLETE.
...
DONE (do NOT redo): Phase 36 Task 2 / gap E1 (live-region announcements), Phase 36 Task 3 / gap E2 (consolidated global glossary & methodology explainer, contract 1.21.0), and Phase 36 Task 4 / gap E3 (reproducibility evidence-pack export, NO contract change) — see `PHASE36_TASK2_E1_REPORT.md`, `PHASE36_TASK3_E2_REPORT.md`, and `PHASE36_TASK4_E3_REPORT.md`. The single next `in_progress` item is **Phase 36 Task 5** (phase summary + consolidated re-audit → PHASE 36 COMPLETE); confirm against `.claude-dev/MODEL_DEV_STATE.json`.
