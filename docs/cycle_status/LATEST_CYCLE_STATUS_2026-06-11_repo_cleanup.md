...
## Pass 3 — redundant root docs (same session; owner-approved scope "Both")

- HISTORICAL/RESOLVED docs DELETED: `GITHUB_PUSH_BLOCKER.md` (incident resolved 2026-06-03; remaining mentions are in historical cycle entries of MODEL_DEV_TASK_PROMPT.md), `IMPLEMENTATION_PLAN_currency_and_inputs.md` (fully implemented as Phase UIL; manual + template live in `production_run/`; the two live citations in production_run/README.md and USER_MANUAL patched to point at git history).
- PATH-2 ONBOARDING family DELETED (automation has been live for weeks): `PATH_2_SETUP_INSTRUCTIONS.md`, `PATH_2_QUICK_REFERENCE.md`, `README_PATH2_INDEX.md`, `QUICK_START_5_STEPS.md`, `ACTUARIAL_MODEL_AUTOMATION_SETUP.md`, `00_READ_ME_FIRST.txt`, `00_START_HERE.md`, `PLACEHOLDERS_GUIDE.md`, `FILE_MANIFEST.md` (stale file list).
- KEPT after reference analysis: ALL `scripts/` (governance/evidence-referenced, incl. both `build_phase24_task3_inner_path_action{,s}.py` variants), all root UI artifacts (`par_projection_gui.html`, `model_result_viewer.html`, `viewer_data.json` are BUILD INPUTS of `scripts/build_combined_gui.py`; `ui_app.html`/`ui_data.json` regenerated by `production_run/build_gui.py`), `MODEL_DEV_STATE_TEMPLATE.json` (recovery aid), `UI_README.md`, `docs/recovery_2026-06-05/` (audit trail).
- Git history retains every deleted file.
